	...
	require_once('middle.php'); ?>
		...
		<?php 
			...
			for ($j=($temp-$page_rows);$j<$temp;$j++) {
				...
				if ($j<$rows)
				{
					...
					echo '
						...
						<button type="button" onclick="AJAXaddtocart(\'',$nama,'\')">Add to cart</button><br><br>
					';
				}
			}	
			...
			if ($pagenum==1) {
				...
				echo '<a href="product.php?pagenum=1&kategori=',$kategori,'&sort=',$sort,'"> <<-First</a> &nbsp;  ';
				$previous = $pagenum-1;
				echo '<a href="product.php?pagenum=',$previous,'&kategori=',$kategori,'&sort=',$sort,'"> <-Previous</a>   &nbsp; ---';
			}		
			if ($pagenum==$last) {
			} else {
				$next = $pagenum+1;
				echo '<a href="product.php?pagenum=',$next,'&kategori=',$kategori,'&sort=',$sort,'"> Next -></a>   &nbsp; ---';
				echo '<a href="product.php?pagenum=',$last,'&kategori=',$kategori,'&sort=',$sort,'"> Last ->></a>   &nbsp;';
			}
		?>
		</div>
	</div>
