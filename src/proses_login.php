...
<?php
	...
	$url= "http://if3110tubes3.herokuapp.com/REST/getPassPelanggan/".$email;
	$response=json_decode(file_get_contents($url));
	$bool=$response;
	
	if($bool)
	{
			$url= "http://if3110tubes3.herokuapp.com/REST/cekPassword/".urlencode("SELECT password FROM customer WHERE username = '$email' and password= '$password'");
			$response=json_decode(file_get_contents($url));
			$jumlah=$response;
			if($jumlah!=0)
			{
				$url= "http://if3110tubes3.herokuapp.com/REST/getAllCustomer/".$email;
