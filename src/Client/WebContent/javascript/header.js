	...
	xmlhttp.onreadystatechange=function()
	{
		if (xmlhttp.readyState==4 && xmlhttp.status==200)
		{
			if(xmlhttp.responseText.charAt(0)=='0'){
				...
				localStorage.wbdadmin = "0";
				var s = "<li><a href=\"profile.jsp\">Welcome "+localStorage.wbduser+"!</a></li>";
				s += "<li><a href=\"index.jsp\" onclick=\"javascript:localStorage.removeItem('wbdadmin');localStorage.removeItem('wbduser');localStorage.removeItem('wbdlogintime');localStorage.removeItem('shoppingbag');\">Logout</a></li>";
...
if(typeof(Storage)!=="undefined"){
	if(localStorage.wbduser && localStorage.wbdlogintime){
		if(hitunghari(Number(localStorage.wbdlogintime),new Date().getTime())<=30){
			...
			localStorage.removeItem('wbdadmin');
			localStorage.clear();
