    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (isLogin) {
            ...
            for (int i = 0; i < s.size(); i++) {
                ...
                B.findByPkRest(Integer.parseInt(id_b));
                ...
                if (stok >= Integer.parseInt((String) session.getAttribute(id_b))) {
                    ...
                    // String q = "update barang set stok = " + stok +
                    // " where id = " + id_b;
                    // DBA.insertQuery(q);
                    Barang.updateRest("id=" + id_b + "&stok=" + stok);
                } else
                    System.out.println("DEBUG, barang melebihi stok");
                session.removeAttribute(id_b);
            }
            String id_u = Helper.getUserId(session).toString();
            // DBA.insertQuery("update account set transaksi = transaksi + 1 where id = "
            // + id_u);
            Profile.updateRest("id=" + id_u + "&transaksi=1");
