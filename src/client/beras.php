...
<head>
	...
	function AddToCart(i){
		...
		//alert("AddToCart.php?action=add&id="+i+"&Jumlah="+Jumlah+"&Kategori=Beras");
	
		xmlhttp.open("GET","AddToCart.php?action=add&id="+i+"&Jumlah="+Jumlah+"&Kategori=Beras",true);
		xmlhttp.send();
...
<body>
	...
	<div id="container">
		...
		<div id="sort-bar" class="frame">
			...
			<button type="button" onClick="location.href='beras.php?SortBy=Nama&f=1&l=10'">Nama</button>
			<button type="button" onClick="location.href='beras.php?SortBy=Harga&f=1&l=10'">Harga</button>
		</div>
		<div id="pagination">
			<ul>
				<?php $n = $_GET["f"];
				 ...
				 <?php }
				 if ($_GET["l"]<count($_SESSION['Beras'])){
					   $n = $_GET["f"]+10;
					   $m = $_GET["l"]+10;
				  ?>
