<?php 
	...
	$con = getConnection();
	$result = mysql_query($con,"SELECT * FROM category");
	...
	while($row = mysql_fetch_array($result))
  	{
		...
		echo "	<ul>";
			$con2 = getConnection();
			$result2 = mysql_query($con2,"SELECT * FROM task WHERE categoryid = ".$row['categoryid']);
			while($row2 = mysql_fetch_array($result2))
  			{
				...
				$con3 = getConnection();
				$result3 = mysql_query($con3,"SELECT tagid FROM task_tag WHERE taskid = '".$taskid."'");
