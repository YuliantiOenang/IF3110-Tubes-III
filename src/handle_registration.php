	...
	function addToUser($data){
		...
		if (!$statement->fetch()){
			...
			if (!$statement1->fetch()){
				...
				$role=$data["role"];
				$statement2 = $conn->prepare("INSERT INTO member(name,address,contact,email,username,password,jumlah_transaksi,role)VALUES('$name', '$address', '$contact', '$email', '$username', '$password', 0, '$role')");
				$statement2->execute();
				$statement2->close();
