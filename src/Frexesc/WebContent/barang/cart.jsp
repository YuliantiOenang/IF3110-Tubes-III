...
<body>
	...
	<%
		...
		ArrayList<BarangUserBean> barangUserBean = (ArrayList<BarangUserBean>) request.getAttribute("user_items");
	%>
	<%
		...
		<div class='orderdata'>
						...
						<div class='row'>
							...
							<%
								for (int j = 0; j < barangBean.size(); j++) {
										...
										if (b2.getId() == b.getId_barang()) {
											totalHarga = totalHarga + b.getJumlah_barang() * b2.getHarga_barang();
							%>
							...
							<div class='list_body' id='price'>
								<p>
									IDR
									<%=b2.getHarga_barang()%></p>
							</div>

							<div class='list_body' id='qty'>
								<input type="number" name="qty" size="8" id="qty_<%=b.getId()%>"
									value="<%=b.getJumlah_barang()%>">
							</div>

							<div class='list_body' id='subtotal'>
								<p>
									IDR <span id="harga_<%=b.getId()%>"><%=b.getJumlah_barang() * b2.getHarga_barang()%>
									</span>
								</p>
							</div>
							...
							<div class='list_body' id='remove'>
								<a href="./deleteBarang?id=<%=b.getId()%>"
									title='Remove <%=b2.getNama_barang()%> from your Shopping Cart'>&nbsp;&nbsp;&nbsp;x</a>
							</div>
			...
			<div class='formcontainer'>
				<div class='buyerdetail'>
					...
					%>
					<a href='./payment'><input type="btn" name="submit"
						value="Process to payment" class="button"></a>
					<%
						}
					%>
					<a href='./'><input type="btn" name="submit" value="Add Item"
						class="button"></a>
				</div>
			</div>
		</div>
