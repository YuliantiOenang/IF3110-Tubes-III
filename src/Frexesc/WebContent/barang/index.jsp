		...
		<div class='itembox'>
			<div class='pict' id='item<%=b.getId()%>'>
				<div title='Ready Stock' class='itembox_img'>
					<img onload='fitbarang(this)'
						src='../img/barang/<%=b.getGambar()%>' />
				</div>
				<div class='minicart_icon'>
					<a href=# onclick='goToCart(<%=b.getId()%>)'><img
						src='../img/site/cart_black.png' /></a>
				</div>
				<div class='item_name'>
					<a href='./detail?id=<%=b.getId()%>'><%=b.getNama_barang()%></a><br />
					IDR
					<%=b.getHarga_barang()%>
				</div>
			</div>
			...
			<div class='minicart hidden' id='cart<%=b.getId()%>'>
				<label class='qty small'>Quantity</label> <input type='number'
					name='qty' id='qty_<%=b.getId()%>' class='qty' value=0></input> <label
					class='qty small'>Stock: &nbsp;&nbsp;<span
					id="jumlah_barang_<%=b.getId()%>"><%=b.getJumlah_barang()%></span>
				</label><br />
				<p>
					<label class='rqmessage'>Request Message :</label>
				</p>
				<textarea class='req_msg small' name='deskripsi_tambahan'
					id='deskripsi_tambahan'></textarea>
				<input type='button' class='cart small' id='beli'
					value='Add to Cart'
