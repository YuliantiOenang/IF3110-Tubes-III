...
<body>
	...
	<div class="table">
		...
		<div class="row" id="<%="r" + b.getId()%>">
			...
			<div class="c2" id="<%="cat" + b.getId()%>"><%=categories[(int) (b.getId_kategori()) - 1]%></div>
			<div class="c3" id="<%="name" + b.getId()%>"><%=b.getNama_barang()%></div>
			<div class="c4" id="<%="pri" + b.getId()%>"><%=b.getHarga_barang()%></div>
			<div class="c5" id="<%="desc" + b.getId()%>"><%=b.getKeterangan()%></div>
			<div class="c6" id="<%="pic" + b.getId()%>">
				<a href="#"
					onclick="window.open('admin?action=pic&id=<%=b.getId()%>', 'newwindow', 'width=500, height=250'); return false;"><%=b.getGambar()%></a>
			</div>
			<div class="c7" id="<%="total" + b.getId()%>"><%=b.getJumlah_barang()%></div>
			<div class="c8" id="<%="edit" + b.getId()%>">
				<button onclick="editbarang(<%=b.getId()%>)">edit</button>
			</div>
			<div class="c9">
				<form method="post" action="admin"
					onsubmit='return confirmdelete("<%=b.getNama_barang()%>")'>
