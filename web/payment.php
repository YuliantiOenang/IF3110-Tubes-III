<?php
  ...
  $url= "http://limitless-earth-2748.herokuapp.com/REST/getCardNumber/".$_COOKIE['IdCustomer'];
...
$arr=$response;
  ...
  $url= "http://limitless-earth-2748.herokuapp.com/REST/prosesPayment1/".$_COOKIE['IdCustomer'];
$response=json_decode(file_get_contents($url));
	$count=0;
$arr=$response;
  $bool=$arr;
			...
			if($bool)
			{
			  $url= "http://limitless-earth-2748.herokuapp.com/REST/prosesPayment2/".$_COOKIE['IdCustomer'];
			...
			{
				$url= "http://limitless-earth-2748.herokuapp.com/REST/prosesPayment3/".$arr['idAlat'.$count];
				$response=json_decode(file_get_contents($url),true);
				$arr2=$response;
				var_dump ($arr2);
				$counter=0;
   ...
   $mysql3=$mysql2['status'];
					...
					$url= "http://limitless-earth-2748.herokuapp.com/REST/updatePeralatan/".$newCount."/".$arr['jumlah'.$count];
					$response=json_decode(file_get_contents($url));
					$mysql4=$response;
					
					$url= "http://limitless-earth-2748.herokuapp.com/REST/deleteKeranjang/".$_COOKIE['IdCustomer'];
					$response=json_decode(file_get_contents($url));
					$mysql5=$response;
					if(!$mysql3 || !$mysql4)
					{
