	...
	<div class = "boddy">
		...
		if(!isset($_GET['key']))
		{
			...
			{	
				...
				$url= "http://limitless-earth-2748.herokuapp.com/REST/getBarangAll/";
		...
		if(!isset($_GET['page']))
		{
			...
			echo '<div class = "searchres">
					...
					<input type="button" onclick="var ask=confirm('Apakah Anda yakin menghapus barang ini ? ');
												if(ask)
												{
												window.location='delete.php?idBarang=<?php echo $_SESSION['id'.$i];?>';
	}" value="Delete"></input>
					</center>
					<?php
					}
		...
		else
		{
			...
			echo '<div class = "searchres">
					...
					<input type="button" onclick="var ask=confirm('Apakah Anda yakin menghapus barang ini ? ');
												if(ask)
												{
												window.location='delete.php?idBarang=<?php echo $_SESSION['id'.$counter];?>';
	}" value="Delete"></input>
					</center>
					<?php
					}
					...
					{
					?>
					<input type="button" onclick="alert('anda harus login terlebih dahulu'); window.location='index.php'" value="Delete"></input>
					<?php
					}
					echo '</div>';
			$counter++;
