...
function check_email_adress(str){
	var i;
	for (i = 0; i < str.length; i++) {
		if (str.charAt(i) == '.' || str.charAt(i) == '_' ||  (str.charCodeAt(i) > 47 && str.charCodeAt(i) < 58) || (str.charCodeAt(i) > 64 && str.charCodeAt(i) < 91) || (str.charCodeAt(i) > 96 && str.charCodeAt(i) < 123)) {
		}
		else
		{
			return false;
...
function check_user_name() {
	...
	if(str!=""){
		if(check_email_adress(str) && str.length>4){
			if(str!=password){
			chkusername=true;
			document.getElementById("warning-message").innerHTML="";
			...
			chkusername=false;
			hide_create_button();
			document.getElementById("warning-message").innerHTML="Username minimal 5 karakter dan tidak mengandung karakter spasi,simbol";
		}
	}
}
...
function check_email() {
	...
	if(str!=""){
		...
		if(emailadress.length>0 && domain.length>0 && domain2.length>1 && check_email_adress(emailadress) && is_only_character(domain) && is_only_character(domain2)) {
			if(str!=password){
			chkemail=true;
			document.getElementById("warning-message").innerHTML="";
			}else{
